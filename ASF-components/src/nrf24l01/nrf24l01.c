...
volatile uint8_t PTX;	// Sending mode

// Timing delay
#define Tpor		150+10	// Delay (ms) of power on reset
#define Tpd2stby	150+10	// Delay (us) after the nRF24L01+ leaves power down mode before the CE is set high
#define Tstby2a		130+10	// RX/TX Settling time (us), delay from Standby to TX/RX mode
#define Tpece2csn	4+1		// Delay (us) from CE positive edge to CSN low
#define Thce		10+2	// Minimum CE high (us)
...
void nrf24l01_set_addr_len(enum nrf24l01_addrlen addr_len)
{
	...
	switch(addr_len)
	{
		case NRF24L01_ADDR_LEN_3:
		nrf24l01_write_register(NRF24L01_SETUP_AW_REG, 0x01);
		break;
		case NRF24L01_ADDR_LEN_4:
		nrf24l01_write_register(NRF24L01_SETUP_AW_REG, 0x02);
		break;
		case NRF24L01_ADDR_LEN_5:
		nrf24l01_write_register(NRF24L01_SETUP_AW_REG, 0x03);
		break;
	}
...
void nrf24l01_send_command(uint8_t command)
{
	...
	spi_write_single_packet(&CONF_NRF24L01_SPI, command);
	spi_deselect_device(&CONF_NRF24L01_SPI, &nrf24l01_spi_device_conf);
}
...
{
	nrf24l01_write_array_register(NRF24L01_RX_ADDR_P1_REG, addr, CONF_NRF24L01_ADDR_LEN);
}
...
void nrf24l01_set_tx_addr(uint8_t* addr)
{
	...
	nrf24l01_write_array_register(NRF24L01_RX_ADDR_P0_REG, addr, CONF_NRF24L01_ADDR_LEN);
	nrf24l01_write_array_register(NRF24L01_TX_ADDR_REG, addr, CONF_NRF24L01_ADDR_LEN);
}
...
{
	uint8_t nrfData[CONF_NRF24L01_PAYLOAD];	// Max payload is 32 bytes
	
	// Check if there is data to RX from air
	while(nrf24l01_isSending()){}
	while(!nrf24l01_dataReady()){}
	nrf24l01_receive_data(nrfData);
	
	// Return only first byte, discard others
	return nrfData[0];
}

void nrf24l01_write_byte(uint8_t nrfData)
{
	// Append 0xAA bytes to fill payload
	uint8_t nrfPayloadData[CONF_NRF24L01_PAYLOAD] = { 0xAA };
	nrfPayloadData[0] = nrfData;
	while(nrf24l01_isSending()){}
	nrf24l01_send_data(nrfPayloadData);
}
